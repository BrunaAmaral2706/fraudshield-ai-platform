      ...
      <div className="mt-6 space-y-6">
        ...
        {mlMetrics?.metrics && (
          <div className="rounded-xl border border-slate-200/80 bg-white p-5 shadow-[var(--shadow-card)] dark:border-slate-700 dark:bg-slate-800">
            ...
            {mlMetrics.roc_curve?.length > 0 && (
              <div className="mt-6">
                <h4 className="mb-2 text-[13px] font-semibold text-slate-800 dark:text-white">ROC Curve</h4>
                <div className="h-[220px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={mlMetrics.roc_curve}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="fpr" tick={{ fontSize: 11 }} label={{ value: 'FPR', position: 'insideBottom', offset: -2, fontSize: 10 }} />
                      <YAxis dataKey="tpr" tick={{ fontSize: 11 }} label={{ value: 'TPR', angle: -90, position: 'insideLeft', fontSize: 10 }} />
                      <Tooltip />
