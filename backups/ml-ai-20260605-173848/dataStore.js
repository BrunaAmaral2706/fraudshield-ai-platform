...
const { computeRisk, formatCategoryLabel, getRiskLevel, getAlertLevel, computeStatus } = require('./riskEngine');
const {
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream(CSV_PATH)
      ...
      .on('data', (row) => {
        ...
        const velocityBonus = Math.min(15, ((merchantCounts.get(ccNum) || 1) - 1) * 3);
        const finalScore = Math.min(99, risk.risk_score + velocityBonus);
        const risk_level = getRiskLevel(finalScore);
        const alert_level = getAlertLevel(risk_level);
        const status = computeStatus(amount, finalScore, risk_level);
...
async function initializeData() {
  ...
  cache.loading = (async () => {
    ...
    try {
      ...
      cache.hours = computeHoursFromTransactions(frauds);
    } catch (err) {
